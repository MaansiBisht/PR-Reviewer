    ...
    <li className="animate-fade-in">
      <button
        ...
        className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 group
          hover:bg-surface-800/50 ${expanded ? 'bg-surface-800/30' : ''}`}
      >
        <span className={`p-1.5 rounded-lg ${agent.bgColor} ${agent.color} transition-transform duration-200 group-hover:scale-110`}>
          {agent.icon}
        </span>
        <span className="text-sm flex-1 text-left text-surface-300 group-hover:text-surface-100 transition-colors">
          {agent.role}
        </span>
      ...
      <div className={`overflow-hidden transition-all duration-300 ease-out ${expanded ? 'max-h-24 opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="ml-12 mr-3 mt-1 mb-2 p-3 bg-surface-800/30 rounded-lg text-xs text-surface-400 leading-relaxed">
          {agent.description}
        </div>
      </div>
    </li>
